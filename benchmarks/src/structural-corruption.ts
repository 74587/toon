...
// Peels trailing blank lines (e.g. YAML/XML end with a newline) off the record
...
// Groups YAML sequence items: each record starts at a `  - ` line and runs until
// the next one.
...
// Groups XML records by `<employees>` .. `</employees>` open/close pairs, inclusive.
...
// Groups pretty-printed JSON object literals: each record runs from a `    {`
// line to its matching `    }` line, with any trailing comma stripped.
...
// Drops the last `count` records – TOON keeps its declared `[N]`, so the header
...
// Appends the record blocks from a same-format encoding of the extra rows –
// TOON's `[N]` stays put, so the rows outnumber the declared length.
...
// Detects whether one field line belongs to `fieldName` in a multi-line record.
...
// Removes one field from the targeted records – single-line formats lose a cell
// (TOON/CSV rows go narrower than the header), multi-line formats lose the
// property.
...
/**
 * Corrupts a format's encoded text according to a structural corruption descriptor.
 *
 * @remarks
